...
class ShowPdf():
    ...
    def pdf_view(self,master,width=1200,height=600,pdf_location="",bar=True,load="after"):
        ...
        if bar==True and load=="after":
            self.display_msg = Label(textvariable=percentage_load,bg='#000033')
            self.display_msg.pack(side=TOP,pady=10)

            loading = Progressbar(self.frame,orient= HORIZONTAL,length=100,mode='determinate',style='red.Horizontal.TProgressbar')
            loading.pack(side = BOTTOM,fill=X)
        ...
        def add_img():
            ...
            for page in open_pdf:
                ...
                #img = Image.open(img)
                img = io.BytesIO(img)
                #img = Image.frombytes(mode='RGBA',size=(800,800),data=img)
                #img = Image.open(img)
                #img = img.resize((800,600),Image.LANCZOS)
                #timg = PhotoImage(data = img)
                timg = Image.open(img)
                #timg = Image.open(timg)
                timg = timg.resize((800,1000),Image.LANCZOS)
                timg = ImageTk.PhotoImage(image=timg)
                #timg = PhotoImage(image=timg)
                #timg = timg.zoom(2)
               # timg = timg.subsample(500,500)
                #timg = timg.zoom(2)
                #timg = timg.subsample(2)
            ...
            for i in self.img_object_li:
                ...
                self.text.tag_configure("aligh",justify='center')
                self.text.configure(state="disabled")
                self.text.tag_add('aligh',i,"end")
